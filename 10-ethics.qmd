...
```{r}
library(tidyverse)
challenger_df <- read_csv("data/Challenger.csv")
```
...
__Exercise 3__. Using `modelr`, construct a visualization of the fitted regression model on the probability scale (so not on the odds or log-odds scale). Note that, because this model only has one predictor, you can make this plot fairly easily without `modelr`. However, for practice, use `modelr` to make the plot (make the grid of predictor values, use `gather_predictions()`, etc.).
