...
The Happy Planet Index (HPI) is a measure of how efficiently a country uses its ecological resources to give its citizens long "happy" lives. You can read more about this data <a href="https://happyplanetindex.org/wp-content/themes/hpi/public/downloads/happy-planet-index-briefing-paper.pdf" target="_blank">here</a>.
...
Run the code in the following chunk to read in the data:
...
a. Using this data set, construct a visualization that explores gender balance within each of the majors at SLU. We will begin this exercise by sketching out what we want to create as a class.

b. Suppose that we want to present our visualization to members of the MCSSDS department. How can we modify this visualization to be tailored to this particular audience? 
...
majors_plot <- df |> select(-starts_with("minor")) |>
  ...
  summarise(total_students = n(),
            prop_male = mean(sex_ind)) |>
  filter(total_students >= 45) |>
...
majors_plot <- majors_plot |> mutate(mcss = if_else(major %in% c("STAT", "MATH", "CS",
                                              "DATA"),
                                              true = "mcss",
                                              false = "other"))
...
(majors_plotly <- ggplot(data = majors_plot, aes(x = major, y = prop_male,
                                                 colour = mcss, size = mcss)) +
  geom_segment(aes(xend = major, y = 0, yend = prop_male), show.legend = FALSE) +
  ...
  scale_colour_manual(values = c("maroon", "grey40")) +
...
majors_plot_high <- majors_plot |> filter(F >= 200 | M >= 400)
ggplot(data = majors_plot, aes(x = F, y = M)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0, linetype = 2) +
  geom_text(data = majors_plot_high, aes(label = major))

ggplot(data = majors_plot, aes(x = prop_male, y = total_students)) +
  geom_point()
```
