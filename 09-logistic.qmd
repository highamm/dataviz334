...
To help communicate what a fitted logistic regression model means, in terms of predicted probability (as opposed to predicted odds or log-odds), we will again make use again of the `ggeffects` package. We will use the same overall strategy as the previous section on using `ggeffects` to visualize models from standard linear regression models. 

Our basic strategy for visualizing models is to

1. fit the model with `glm()`.

2. construct a grid of predictor values with the `ggpredict()` function from the `ggeffects` package. 

3. Obtain predicted probabilities for the response variable at the values specified in our grid using `ggpredict()`.
...
We next use `ggeffects` to create a grid of predictors that we want to make predictions for and then obtain predictions at values in the grid:
...
__Exercise 1__. Examine the `predicted` column in `aug_surv`. Can you calculate one of these values using the model output "by hand?"

__Exercise 2__. Make a plot of the values of `predicted` that lets you explore how our predictors are associated with the survival on the Titanic.
...
__Exercise 2__. Obtain model predictions for both the previous model (without the interaction) and the new model (with the interaction).
...
preds_interaction <- ggpredict(titanic_mod_int, terms = c("Age", "Sex", "Pclass")) |>
  as.data.frame(terms_to_colnames = TRUE) |>
  as_tibble() 

plot_df <- bind_rows(lst(preds_titanic_large,
                         preds_interaction), .id = "model")
...
#| output: false
library(broom)
glance(titanic_large)
glance(titanic_mod_int)
```
