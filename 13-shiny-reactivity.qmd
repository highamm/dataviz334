...
**Step 2**: Set up our `shiny` app inputs. Before, we just had a single input. Now, we will have two: one for player and one for variable. Let's focus on one at a time, doing `player` first. Type `shinyapp` and click on the `R Studio` snippet to bring up a base app. Then, add `player` as an input in the UI.
