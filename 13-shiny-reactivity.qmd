...
**Step 2**: Set up our `shiny` app inputs. Before, we just had a single input. Now, we will have two: one for player and one for variable. Let's focus on one at a time, doing `player` first. Type `shinyapp` and click on the `R Studio` snippet to bring up a base app. Then, add `player` as an input in `sidebarPanel()` part of a `sidebarLayout()` UI.
...
**Step 5**: We can then repeat some of these steps for a second input: a variable that the user selects. We will use `radioButtons()` as the input in the UI.

We will discuss why we need to take some extra steps to perform the "user can select a variable" operation in class. In particular, we will need to briefly discuss <a href="https://mastering-shiny.org/action-tidy.html" target="_blank">tidy evaluation</a> to use some of the `tidyverse` functions in `shiny` (see @sec-tidyeval).

**Step 6**: Add a third input that will let the user change the number of bins in the histogram.

**Step 7**: Use the `glue()` function from the `glue` package to change the title of the histogram to include the name of the player in the plot (see @sec-glue).

**Step 8**: Suppose that we want to add a buttons that the user can select corresponding to the two main professional tennis tours (`atp` for the men's tour and `wta` for the women's tour). After the user selects an appropriate tour, we want the player choices to give players corresponding __only__ to that tour (so that only men appear when `atp` is selected and only women appear when `wta` is selected). To make something like this happen, we need to learn about how to make the UI __dynamic__ (@sec-dynamicui).
...
__Exercise 4__. The following app UI was taken from the *Mastering Shiny* book. With the app skeleton, complete the following.
...
a. Complete the server function so that the country choices update according to the continent that the user selects. Have the app display a table with the corresponding rows to the country that the user then selects.

b. Add an action button so that the table only updates after the user clicks the button (see @sec-action_button).

c. Extend the app so that there is an option to choose "All" for continent, which lets you see all of the countries in the list. To do this, you can use a base `R` `if else` statement (see @sec-ifelse) to trigger the "special" case for when "All" is selected. You can also use this code to get "All" to appear as an option in your list of choices for the continent:
...
### Dynamic UI {#sec-dynamicui}

For what we have learned about so far, we have set up a Shiny app so that, when the user changes something in the user interface (UI), something else in the server changes. But suppose that we would the __UI__ itself to update when the user changes something in the UI. If we want the UI to update based on what a user selects, then we want the UI to be __dynamic__.

As an example, suppose that in the previous Pokemon example, we want the user to choose a pokemon `Generation` (1, 2, 3, 4, 5, 6) and to have our list only contain Pokemon for that particular Generation. This situation calls for dynamic UI because our `selectizeInput` UI component should update when the user chooses a `Generation` in the UI.
...
ui <- fluidPage(
  sidebarLayout(
    sidebarPanel(
      selectInput("genchoose", label = "Choose a Generation",
...
server <- function(input, output, session) {
  ...
  observeEvent(input$genchoose, {
    pokemon_choices <- poke_df |> 
      filter(Generation == input$genchoose) |>
      pull(Name) ## makes the Name a vector
    updateSelectizeInput(inputId = "pokechoose1", choices = pokemon_choices)
  })
...
In the code above, there are a few new pieces we added to get the UI to update dynamically:

* we added a `selectInput` piece in the UI so that a user can select a `Generation`.
* we added an `observeEvent` piece in the server. The first argument of `observeEvent` gives the event that we want shiny to "watch for". In this case, the event that we are watching for is a change to the "choose generation" input.
* when `input$genchoose` is changed by the user, Shiny "observes this event," which triggers the code in the `{}` to run.
* the code within the `{}` filters the pokemon data set according to the `Generation` selected and puts all of the pokemon `Name`s from this `Generation` into a vector.
* finally, the `updateSelectizeInput` piece tells the `selectizeInput` in the UI to update with these new choices.

### Action Button {#sec-action_button}
...
The code below adds `actionButton` to the UI, paired with `eventReactive` in the server, to the following example so that the user compare the statistics of a few different Pokemon. But, the graph will not update until the user clicks the action button!
