...
However, because the course is neither a pre-requisite nor a co-requisite for this course, we will just give a short example here. Additionally, you will not be assessed on anything in this subsection. If you decide to use a statistical learning method for your final project, you should be able to use a search engine to help you see how others have visualized that particular method. 

For the first short example, examine some code below that has been modified from Dr. Torrey. The model fit here uses the `ns()` function from the `splines` package. Notice that, besides the addition of `ns()` to the `lm()` function, the remaining steps in visualizing a "grid" of predictions remains the same. __`ggpredict()` can be used with many statistical learning algorithms that have a quantitative response, including smoothing splines, lasso regression, and ridge regression__.

```{r}
library(tidyverse)
library(splines)
Auto <- read_csv("https://raw.githubusercontent.com/selva86/datasets/master/Auto.csv",
...
gam_preds <- ggpredict(gam.lm, terms = c("weight", "year [threenum]", "origin")) |> 
  as.data.frame(terms_to_colnames = TRUE) |>
  as_tibble()


ggplot(data = gam_preds, aes(x = weight, y = predicted)) +
